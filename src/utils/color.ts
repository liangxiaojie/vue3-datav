 ...
 */
function getColorByKeyword(keyword: string) {
  if (!keyword) {
 ...
 */
export function getRgbValue(color: any) {
  if (!color) {
 ...
 */
function getRgbValueFromHex(color: any) {
  color = color.replace('#', '')

  if (color.length === 3)
    color = Array.from(color)
      .map((hexNum: any) => hexNum + hexNum)
      .join('')
 ...
 */
function getRgbValueFromRgb(color: any) {
  return color
    .replace(/rgb\(|rgba\(|\)/g, '')
    .split(',')
    .slice(0, 3)
 ...
 */
export function getRgbaValue(color: any) {
  if (!color) {
 ...
 */
export function getOpacity(color: any) {
  if (!color) {
 ...
 */
export function toRgb(color: any, opacity: any) {
  if (!color) {
...
export function toHex(color: any) {
  ...
  return (
    '#' +
    color
      .map((n: any) => Number(n).toString(16))
      .map((n: any) => (n === '0' ? '00' : n))
      .join('')
  )
}
 ...
 */
export function darken(color: any, percent = 0) {
  if (!color) {
  ...
  rgbaValue = rgbaValue.map((v: any, i: any) => (i === 3 ? v : v - Math.ceil(2.55 * percent))).map((v: any) => (v < 0 ? 0 : v))
 ...
 * @return {String|Boolean} Rgba color (Invalid input will return false)
 */
export function lighten(color: any, percent = 0) {
  if (!color) {
  ...
  rgbaValue = rgbaValue.map((v: any, i: any) => (i === 3 ? v : v + Math.ceil(2.55 * percent))).map((v: any) => (v > 255 ? 255 : v))
